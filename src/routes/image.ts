...
router.get('/', async (req, res) => {
  ...
  const format = req.query.format ? (req.query.format as string).toLowerCase() : '';
  ...
  if (name.length == 0 || isNaN(width) || width < 0 || isNaN(height) || height < 0 || (format.length > 0 && !Object.values(validFormats).includes(format))) {
